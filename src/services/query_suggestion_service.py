...
# from nltk.tokenize import word_tokenize # **إضافة: استيراد word_tokenize مباشرة**
...
# class QuerySuggestionService:
#     def __init__(self, dataset_name, preprocessor, db_connector):
#         self.dataset_name = dataset_name
#         self.preprocessor = preprocessor
#         self.embedding_model = Embedding_online.__loadModelInstance__() 
#         self.db_connector = db_connector 
...
#         SAMPLE_DOCS_FOR_NGRAMS = 0.05  # Reduced from 0.20 for faster processing
#         print(f"Loading cleaned documents (sample_ratio={SAMPLE_DOCS_FOR_NGRAMS}) from database for N-grams...")
#         N_GRAM_PROCESSING_BATCH_SIZE = 2000  # Reduced from 10000 for faster processing
#         docs_list_for_ngrams = self.db_connector.get_all_document_ids_and_texts(
#             self.dataset_name, 
#             cleaned=True, 
#             sample_ratio=SAMPLE_DOCS_FOR_NGRAMS, 
...
#             # Parallelized N-gram extraction
#             def extract_ngrams(args):
#                 text, source_id, source_type = args
#                 tokens = word_tokenize(text.strip())
#                 ngrams = []
#                 # Unigrams
#                 for token in tokens:
#                     ngrams.append((token, source_id, source_type))
#                 # Bigrams
#                 if len(tokens) > 1:
#                     for j in range(len(tokens) - 1):
#                         ngrams.append((f"{tokens[j]} {tokens[j+1]}", source_id, source_type))
#                 # Trigrams
#                 if len(tokens) > 2:
#                     for j in range(len(tokens) - 2):
#                         ngrams.append((f"{tokens[j]} {tokens[j+1]} {tokens[j+2]}", source_id, source_type))
#                 return ngrams

#             current_batch_ngrams_meta = {}
#             with ThreadPoolExecutor(max_workers=24) as executor:
#                 all_ngrams = executor.map(extract_ngrams, batch_texts_with_source_info)
#                 for ngram_list in all_ngrams:
#                     for gram_text, source_id, source_type in ngram_list:
#                         meta = current_batch_ngrams_meta.setdefault(gram_text, {'sources': set(), 'count': 0})
#                         if len(meta['sources']) < MAX_SOURCES_PER_NGRAM:
...
#             # Limit the number of N-grams embedded per batch to 50,000 most frequent
#             MAX_NGRAMS_TO_EMBED_PER_BATCH = 50000
#             sorted_batch_ngrams = sorted(current_batch_ngrams_meta.items(), key=lambda item: item[1]['count'], reverse=True)
#             batch_ngrams_texts = []
#             batch_ngrams_ids = []
#             batch_ngrams_metadatas = []
...
#             print(f"Embedding step for batch {i//N_GRAM_PROCESSING_BATCH_SIZE + 1} took {embedding_end_time - embedding_start_time:.2f} seconds.")
...
#             print(f"Processed source batch {i//N_GRAM_PROCESSING_BATCH_SIZE + 1}. Total N-grams added so far: {total_ngrams_added}. Batch time: {batch_duration:.2f} seconds.")
...
#     def _extract_snippet(self, full_text_from_db: str, phrase: str, window_size: int = 50) -> str:
#         if not full_text_from_db or not phrase:
#             return ""
...
#             snippet_start = max(0, start_index - window_size)
#             snippet_end = min(len(full_text_from_db), end_index + window_size)

#             snippet = full_text_from_db[snippet_start:snippet_end]
            
#             if snippet_start > 0:
#                 snippet = "..." + snippet
#             if snippet_end < len(full_text_from_db):
#                 snippet = snippet + "..."
